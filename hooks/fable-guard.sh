...
# check whether the session's model drifted off Opus 5.5. Hooks CANNOT switch the live
# session model (settings.json "model" is read at startup only), so on drift we:
#   1) self-heal ~/.claude/settings.json "model" back to "claude-opus-5-5" (fixes NEW sessions)
#   2) inject additionalContext so Claude opens its reply telling the user to run
#      /model opus (one keystroke fix for the CURRENT session).
set -uo pipefail
input="$(cat)"
...
[ -n "$MODEL" ] || exit 0
case "$MODEL" in *opus-5-5*) exit 0 ;; esac   # already on Opus 5.5 — nothing to do

# 1) heal the startup default so new sessions come up on Opus 5.5
if [ -f "$SETTINGS" ]; then
...
printf '{"hookSpecificOutput":{"hookEventName":"UserPromptSubmit","additionalContext":"MODEL DRIFT: this session is running on %s, expected Opus 5.5. Begin your reply with one bold line telling the user to type /model opus to switch back. Default model in settings.json has been re-set to Opus 5.5 for new sessions. (Interval check, every 5th message.)"}}' "$MODEL"
exit 0
