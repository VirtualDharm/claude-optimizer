        ...
        try {
            ...
            if (widgetText) {
                ...
                if (widget.type === 'custom-command' && widget.preserveColors) {
                    ...
                    // Honor maxWidth for any widget (not just custom-command) so
                    // long values (e.g. git branch names) can be capped.
                    let normalText = widgetText;
                    if (widget.maxWidth && widget.maxWidth > 0 && getVisibleWidth(normalText) > widget.maxWidth) {
                        normalText = truncateStyledText(normalText, widget.maxWidth, { ellipsis: true });
                    }
                    elements.push({
                        content: applyColorsWithOverride(normalText, widget.color ?? defaultColor, widget.backgroundColor, widget.bold),
                        type: widget.type,
                        widget
                    });
                }
